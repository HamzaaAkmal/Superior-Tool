...
function populateDashboardClassSelector(sections) {
    ...
    if (selectedClass && sections.includes(selectedClass)) {
        ...
        // Automatically set today's day and load schedule
        autoSetTodayForSchedule();
...
function updateDashboardForClass() {
    ...
    if (selectedClass) {
        ...
        // Automatically update day schedule if a day is selected
        const daySelector = document.getElementById('day-selector');
        if (daySelector && daySelector.value) {
            loadDaySchedule();
        } else {
            // If no day is selected, automatically set today's day
            autoSetTodayForSchedule();
        }
        ...
        // Clear day schedule when no class is selected
        clearDaySchedule();
    }
}
...
// Automatically set today's day when class is first selected
function autoSetTodayForSchedule() {
    const today = new Date();
